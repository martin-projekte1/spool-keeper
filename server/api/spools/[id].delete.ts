...
defineRouteMeta({
  openAPI: {
    ...
    description:
      "Deletes a spool. Fails with 409 if it is the last spool on the filament.",
...
export default defineEventHandler(async (event) => {
  ...
  const [countRow] = await db
    ...
    .from(spools)
    .where(
      and(eq(spools.filamentId, target.filamentId!), eq(spools.userId, userId)),
    );

  if ((countRow?.total ?? 0) <= 1) {
    throw createError({
      statusCode: 409,
      statusMessage: "Cannot delete the last spool on a filament",
    });
  }
