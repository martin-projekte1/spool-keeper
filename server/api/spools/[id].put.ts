...
defineRouteMeta({
  openAPI: {
    ...
    description:
      "Updates status, remaining weight, or purchase date of a spool. Only provided fields are changed.",
    ...
    requestBody: {
      required: true,
      content: { "application/json": { schema: s.spoolUpdate } },
    },
    responses: {
      200: {
...
export default defineEventHandler(async (event) => {
  ...
  const [result] = await db
    ...
    .set({
      ...
      ...(body.remainingWeightG !== undefined && {
        remainingWeightG: body.remainingWeightG,
      }),
