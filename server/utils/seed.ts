...
export async function seedForUser(userId: string) {
  ...
  await db.delete(featuresTable).where(eq(featuresTable.userId, userId))
  await db.delete(colors).where(eq(colors.userId, userId))

  const [elegoo] = await db.insert(manufacturers).values({ userId, name: 'Elegoo' }).returning()
  const [esun] = await db.insert(manufacturers).values({ userId, name: 'eSUN' }).returning()
  const [polymaker] = await db.insert(manufacturers).values({ userId, name: 'Polymaker' }).returning()
  const [bambu] = await db.insert(manufacturers).values({ userId, name: 'Bambu Lab' }).returning()

  const [pla] = await db.insert(materials).values({ userId, name: 'PLA' }).returning()
  const [asa] = await db.insert(materials).values({ userId, name: 'ASA' }).returning()

  const [hf] = await db.insert(featuresTable).values({ userId, name: 'HF' }).returning()
  const [plus] = await db.insert(featuresTable).values({ userId, name: 'Plus' }).returning()
  const [matte] = await db.insert(featuresTable).values({ userId, name: 'Matte' }).returning()

  const [black] = await db.insert(colors).values({ userId, name: 'Black', hex: '#111111' }).returning()
  const [gray] = await db.insert(colors).values({ userId, name: 'Gray', hex: '#8a8a8a' }).returning()
  const [lavaRed] = await db.insert(colors).values({ userId, name: 'Lava Red', hex: '#cf3c3c' }).returning()
  const [armyGreen] = await db.insert(colors).values({ userId, name: 'Army Green', hex: '#4a5c3a' }).returning()
  const [green] = await db.insert(colors).values({ userId, name: 'Green', hex: '#3a7d44' }).returning()
  ...
  const [fil1] = await db.insert(filaments).values({
  ...
  const [fil2] = await db.insert(filaments).values({
  ...
  const [fil3] = await db.insert(filaments).values({
