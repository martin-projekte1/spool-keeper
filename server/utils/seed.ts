...
export async function seedForUser(userId: string) {
  db.transaction((tx) => {
    ...
    const elegoo = tx
      ...
      .values({ userId, name: "Elegoo" })
      .returning()
      .get();
    const esun = tx
      .insert(manufacturers)
      .values({ userId, name: "eSUN" })
      .returning()
      .get();
    const polymaker = tx
      .insert(manufacturers)
      .values({ userId, name: "Polymaker" })
      .returning()
      .get();
    const bambu = tx
      .insert(manufacturers)
      .values({ userId, name: "Bambu Lab" })
      .returning()
      .get();

    const pla = tx
      .insert(materials)
      .values({ userId, name: "PLA" })
      .returning()
      .get();
    const asa = tx
      .insert(materials)
      .values({ userId, name: "ASA" })
      .returning()
      .get();

    const hf = tx
      .insert(featuresTable)
      .values({ userId, name: "HF" })
      .returning()
      .get();
    const plus = tx
      .insert(featuresTable)
      .values({ userId, name: "Plus" })
      .returning()
      .get();
    const matte = tx
      .insert(featuresTable)
      .values({ userId, name: "Matte" })
      .returning()
      .get();

    const black = tx
      .insert(colors)
      .values({ userId, name: "Black", hex: "#363636" })
      .returning()
      .get();
    const gray = tx
      .insert(colors)
      .values({ userId, name: "Gray", hex: "#8a8a8a" })
      .returning()
      .get();
    const lavaRed = tx
      .insert(colors)
      .values({ userId, name: "Lava Red", hex: "#cf3c3c" })
      .returning()
      .get();
    const armyGreen = tx
      .insert(colors)
      .values({ userId, name: "Army Green", hex: "#4a5c3a" })
      .returning()
      .get();
    const green = tx
      .insert(colors)
      .values({ userId, name: "Green", hex: "#3a7d44" })
      .returning()
      .get();
    ...
    if (
      ...
      throw createError({
        statusCode: 500,
        message: "Seed lookup data could not be created",
      });
    }
    ...
    if (!fil1 || !fil2 || !fil3 || !fil4 || !fil5) {
      throw createError({
        statusCode: 500,
        message: "Filaments could not be created",
      });
    }
