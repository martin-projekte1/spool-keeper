...
export async function assertOwnedFilamentId(
  userId: string,
  filamentIdInput: unknown,
): Promise<number> {
...
export async function validateOwnedFilamentReferences(
  ...
  const materialId =
    refs.materialId == null ? null : normalizeId(refs.materialId, "materialId");
  const manufacturerId =
    refs.manufacturerId == null
      ? null
      : normalizeId(refs.manufacturerId, "manufacturerId");
  const colorId =
    refs.colorId == null ? null : normalizeId(refs.colorId, "colorId");

  if (materialId !== null) await assertOwnedId(materialId, userId, "material");
  if (manufacturerId !== null)
    await assertOwnedId(manufacturerId, userId, "manufacturer");
  ...
  const normalizedFeatureIds = Array.from(
    new Set(
      rawFeatureIds.map((featureId) => normalizeId(featureId, "featureId")),
    ),
  );
  ...
  if (normalizedFeatureIds.length !== rawFeatureIds.length) {
    throw createError({
      statusCode: 400,
      statusMessage: "featureIds must not contain duplicates",
    });
  }
  ...
  if (normalizedFeatureIds.length > 0) {
    const featureCountRows = await db
      ...
      .where(
        and(
          ...
          inArray(featuresTable.id, normalizedFeatureIds),
        ),
      );
    ...
    if (ownedFeatureCount !== normalizedFeatureIds.length) {
      throw createError({
        statusCode: 403,
        statusMessage: "One or more features do not belong to current user",
      });
    }
  }
...
export async function assertNoFilamentUsage(
  ...
  if (kind === "feature") {
    usedBy = await db
      ...
      .innerJoin(
        filamentFeatures,
        eq(filaments.id, filamentFeatures.filamentId),
      )
      .where(
        and(eq(filamentFeatures.featureId, id), eq(filaments.userId, userId)),
      );
  } else {
    const col = {
      material: filaments.materialId,
      manufacturer: filaments.manufacturerId,
      color: filaments.colorId,
    }[kind];
    usedBy = await db
      .select({ id: filaments.id, name: filaments.name })
      .from(filaments)
