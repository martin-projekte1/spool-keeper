...
async function selectFilamentLookupRows(userId: string, id?: number) {
  return db
    ...
    .leftJoin(
      manufacturers,
      and(
        eq(filaments.manufacturerId, manufacturers.id),
        eq(manufacturers.userId, userId),
      ),
    )
    .leftJoin(
      materials,
      and(eq(filaments.materialId, materials.id), eq(materials.userId, userId)),
    )
    .leftJoin(
      colors,
      and(eq(filaments.colorId, colors.id), eq(colors.userId, userId)),
    )
    .where(
      id == null
        ? eq(filaments.userId, userId)
        : and(eq(filaments.id, id), eq(filaments.userId, userId)),
    );
}

type FilamentLookupRow = Awaited<
  ReturnType<typeof selectFilamentLookupRows>
>[number];
...
async function hydrateFilamentRows(userId: string, rows: FilamentLookupRow[]) {
  ...
  const [allSpools, featRows] = await Promise.all([
    db
      ...
      .from(spools)
      .where(
        and(inArray(spools.filamentId, filamentIds), eq(spools.userId, userId)),
      ),
    db
      .select({
        filamentId: filamentFeatures.filamentId,
        feature: featuresTable,
      })
      .from(filamentFeatures)
      .innerJoin(
        featuresTable,
        eq(filamentFeatures.featureId, featuresTable.id),
      )
      .where(
        and(
          inArray(filamentFeatures.filamentId, filamentIds),
  ...
  return rows.map((row) => ({
    ...
    color: row.colors,
    features: (featuresByFilament.get(row.filaments.id) ?? []).map(
      (row) => row.feature,
    ),
...
export async function getFilamentForUser(userId: string, id: number) {
  const [filament] = await hydrateFilamentRows(
    userId,
    await selectFilamentLookupRows(userId, id),
  );
