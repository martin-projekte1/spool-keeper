...
const { data: filamentsRaw, refresh } =
...
const materialOptions = computed(() => {
  ...
  return [
    ...base,
    ...materialsData.value.map((m) => ({ label: m.name, value: m.id })),
  ];
});
...
const manufacturerOptions = computed(() => {
  ...
  const unique = new Set(
    filamentsRaw.value.map((f) => f.manufacturer?.name).filter(Boolean),
  );
  return [
    ...base,
    ...Array.from(unique)
...
const featureOptions = computed(() => {
  ...
  return featuresData.value.map((f) => ({
    label: f.name,
    value: String(f.id),
  }));
});
...
const filaments = computed(() => {
  ...
  return filamentsRaw.value
    .filter((f) => {
      const matchMaterial =
        !materialFilter.value || f.materialId === materialFilter.value;
      const searchTarget = [
        f.name,
        f.material?.name,
        f.color?.name,
        ...
        .toLowerCase();
      const matchSearch =
        !search.value || searchTarget.includes(search.value.toLowerCase());
      const matchManufacturer =
        !manufacturerFilter.value ||
        f.manufacturer?.name === manufacturerFilter.value;
      const filamentFeatureIds = f.features.map((ft) => String(ft.id));
      const matchFeatures =
        featureFilter.value.length === 0 ||
        featureFilter.value.every((fid) => filamentFeatureIds.includes(fid));
    ...
    .map((f) => {
      ...
      const mappedStatus =
        rawStatus && rawStatus in statusMap
      ...
      return {
        ...
        totalWeightG:
          f.spools?.reduce((s, sp) => s + (sp.initialWeightG ?? 0), 0) ?? 0,
        remainingWeightG:
          f.spools?.reduce((s, sp) => s + (sp.remainingWeightG ?? 0), 0) ?? 0,
      };
    });
});
