...
const selectedColorHex = computed(() => {
  ...
  return colors.value?.find(c => c.id === props.modelValue.colorId)?.hex ?? null
})
</script>
  ...
  <div class="space-y-4">
    <UFormField label="EAN">
      <div class="flex gap-2">
        <UInput v-model="modelValue.ean" class="flex-1" placeholder="Scan or type EAN / QR…" />
        <UButton icon="i-lucide-scan-barcode" variant="outline" @click="emit('open-scanner')">
    ...
    <div class="grid grid-cols-2 gap-4">
      <UFormField class="col-span-2" label="Filament Name">
        <UInput v-model="modelValue.name" class="w-full" placeholder="e.g. Rapid PLA+, ecoPLA…" required />
      </UFormField>
    ...
    <UFormField label="Features">
      <div class="flex flex-wrap gap-3">
        ...
        >
          <input v-model="modelValue.featureIds" :value="feat.id" class="rounded" type="checkbox" />
          {{ feat.name }}
        </label>
      </div>
    </UFormField>
    ...
    <div class="border-t border-default pt-4 space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        <UFormField label="Min Temp (°C)">
          <UInput v-model="modelValue.printTempMin" max="350" min="150" required type="number" />
        </UFormField>
        <UFormField label="Max Temp (°C)">
          <UInput v-model="modelValue.printTempMax" max="350" min="150" required type="number" />
        </UFormField>
      </div>
      ...
      <UFormField label="Image">
        <div class="space-y-2">
          ...
          <label class="block">
            ...
            <input accept="image/*" class="hidden" type="file" @change="emit('image-change', $event)" />
          </label>
        </div>
      </UFormField>
    </div>
